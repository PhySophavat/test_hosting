...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold px-4 py-2 border-b font-medium mb-10 text-gray-900">បញ្ជីថ្នាក់</h1>

    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        @foreach ($grades as $grade => $sections)
            <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all">
                <h2 class="text-lg font-semibold text-center mb-3 text-blue-600">ថ្នាក់ {{ $grade }}</h2>
                ...
                <div class="flex flex-col gap-2">
                    @foreach ($sections as $section)
                        ...
                        @if($hasPermission)
                            <a href="{{ route('class.show', $className) }}" 
                               class="block rounded-lg border border-blue-200 text-blue-600 bg-blue-50 hover:bg-blue-100 hover:border-blue-300 text-center font-medium py-2 transition-all">
                                {{ $className }}
                            </a>
                        @else
